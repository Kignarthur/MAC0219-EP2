...
void init(int argc, char *argv[]){
    if(argc < 7){
        printf("usage: ./mandel_ompi_pth c_x_min c_x_max c_y_min c_y_max image_size num_threads\n");
        printf("examples with image_size = 11500:\n");
        printf("    Full Picture:         ./mandel_ompi_pth -2.5 1.5 -2.0 2.0 11500 4\n");
        printf("    Seahorse Valley:      ./mandel_ompi_pth -0.8 -0.7 0.05 0.15 11500 16\n");
        printf("    Elephant Valley:      ./mandel_ompi_pth 0.175 0.375 -0.1 0.1 11500 32\n");
        printf("    Triple Spiral Valley: ./mandel_ompi_pth -0.188 -0.012 0.554 0.754 11500 64\n");
        exit(0);
    }
    else{
...
void *compute_mandelbrot(void *arguments){
    ...
    int y_start_process = taskid * IMAGE_HEIGHT / PROCESSES;
    int y_end_process = (taskid+1) * IMAGE_HEIGHT / PROCESSES;

    int chunk = (y_end_process - y_start_process);

    int y_start = y_start_process + (tid * chunk / NUM_THREADS);
    int y_end = y_start_process + ((tid+1) * chunk / NUM_THREADS);
...
int main(int argc, char *argv[]){
    ...
    for(long t = 0; t < NUM_THREADS; t++){
        args[t].tid = t;
        args[t].taskid = taskid;
        pthread_create(&threads[t], NULL, compute_mandelbrot, (void *) &args[t]);
    }
